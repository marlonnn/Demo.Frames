...
namespace Demo.Frames
{
    public partial class FrameForm : System.Windows.Forms.Form
    {
        ...
        private void SaveVideoProgressHandler(double progress)
        {
            if (this.progressPanel2.ProgressBar.InvokeRequired)
            {
                SaveVideoProgress d = new SaveVideoProgress(SaveVideoProgressHandler);
                this.progressPanel2.ProgressBar.Invoke(d, new object[] { progress });
            }
            else
            {
                int value = (int)(100 * (progress + 1) / this._grabFrames.Frames.Count);
                if (value >= 100)
                {
                    value = 100;
                    this.progressPanel2.IsVisible(false);
                    MessageBox.Show("成功导出全部帧图！", "提示", MessageBoxButtons.OK, MessageBoxIcon.Information);
                }
                this.progressPanel2.ProgressBar.Value = value;
            }
        }
        ...
        private void btnVideoSave_Click(object sender, EventArgs e)
        {
            using (FolderBrowserDialog dialog = new FolderBrowserDialog())
            {
                dialog.Description = "请选择将要导出视频的文件夹";
                if (dialog.ShowDialog() == DialogResult.OK)
                {
                    if (this._grabFrames != null && this._grabFrames.Frames != null)
                    {
                        this.progressPanel2.IsVisible(true);
                        videoDir = dialog.SelectedPath;
                        AviManager aviManager = new AviManager(videoDir + @"\new.avi", false);
                        SaveVideo saveVideo = new Demo.Frames.SaveVideo(this._grabFrames, aviManager);
                        saveVideo.SaveVideoProgressHandler += SaveVideoProgressHandler;
                        saveVideo.ExportVideo();
                    }
